<template>
  <div class="max-w-2xl mx-auto py-10">
    ...
    <p class="mb-4 text-gray-600 dark:text-gray-300">{{ pageDescription }}</p>
    <p class="mb-4 text-sm font-semibold text-primary">{{ batchMessage }}</p>
    <p class="mb-4 text-sm text-gray-600 dark:text-gray-300">{{ localMessage }}</p>
    <div class="mb-6">
      <input type="file" multiple :accept="acceptTypes" @change="onFileChange" class="mb-2" />
    ...
    <div v-if="downloadUrl" class="mt-6">
      <div class="p-3 border rounded bg-green-50 dark:bg-green-900 text-green-800 dark:text-green-200 flex items-center">
        ...
        <span class="mr-2">{{ resultsCount > 1 ? batchSuccessMessage : t('imageConvert.success') }}</span>
    ...
    <div class="mt-8 p-4 border rounded text-sm dark:text-gray-100">
      {{ t('webpTo.intro', { format: targetFormat.toUpperCase() }) }}
    </div>
  </div>
</template>
...
const formatKey = computed(() => targetFormat.value === 'jpeg' ? 'jpg' : targetFormat.value)
const pageTitle = computed(() => t('webpTo.title', { format: formatKey.value.toUpperCase() }))
const pageDescription = computed(() => t('webpTo.description', { format: formatKey.value.toUpperCase() }))
const pageKeywords = computed(() => t('webpTo.keywords', { format: formatKey.value.toUpperCase() }))
const batchMessage = computed(() => t('webpTo.batch_message') || 'Select multiple WebP images and download batch results as a ZIP file.')
const localMessage = computed(() => t('webpTo.local_message') || 'Files are processed locally in your browser and are never uploaded.')
